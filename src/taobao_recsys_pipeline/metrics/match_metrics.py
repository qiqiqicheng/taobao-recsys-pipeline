...
class MatchMetrics(torchmetrics.Metric):
    """
    ...
    def update(self, top_k_ids: torch.Tensor, target_ids: torch.Tensor, **kwargs):
        self.top_k_ids.append(top_k_ids)  # type: ignore
        self.target_ids.append(target_ids)  # type: ignore

    def compute(self):
        # Concatenate the lists of tensors
        top_k_ids = torchmetrics.utilities.dim_zero_cat(self.top_k_ids)  # type: ignore
